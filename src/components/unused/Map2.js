...
class WorldMap extends Component {
  constructor () {
    ...
    this.state = {
      ...
      cities: [
        ...
        { name: 'Sydney', coordinates: [151.2093, -33.8688], population: 5000000 }
      ]
    }
  ...
  handleCountryClick (countryIndex, d) {
    console.log('Clicked on country: ', this.state.worlddata[countryIndex])
    this.setState({ selectedId: countryIndex })
    console.log(d)
    console.log(countryIndex)
  }
  handleCountryHover (countryIndex, d) {
    this.setState({ hoverCountry: countryIndex })
  }
  handleMarkerClick (i) {
        ...
        <g className='countries'>
          {
            this.state.worlddata.map((d, i) => (
              <path
                ...
                className='country'
                fill={i == this.state.selectedId ? 'red' : i == this.state.hoverCountry ? 'darkgrey' : 'grey'}
                stroke='#FFFFFF'
                strokeWidth={0.5}
                onClick={() => this.handleCountryClick(i, d)}
                onMouseOver={() => this.handleCountryHover(i, d)}
              />
            ))
          }
        </g>
